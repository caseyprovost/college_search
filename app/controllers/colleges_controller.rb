class CollegesController < ApplicationController
  ...
  def fetch_remote_colleges(api_key, school_name)
    begin
      response = HTTParty.get("https://api.data.gov/ed/collegescorecard/v1/schools.json?api_key=#{api_key}&school.name=#{school_name}&fields=ope6_id,school.name,location.lat,location.lon", headers: { 'Content-Type' => 'application/json' })
